...
</svg>` /* icon de suppression d'article */
...
</svg>` /* icon de livraison */

if(!produit){ /* cacher le tableau de commande et formulaire si il n'ya pas de produit selectionné */
...
}else{ /* cacher la photo représentant le panier vide */
  document.getElementById("emptyTable").style.display = "none"
  var nombreDeProduit = produit.length
  for (i =0 ; i < nombreDeProduit; i++){ /* insertion des données dans la tableau de commande */
  ...
  if (totale/100 >=100){ /* livraison gratuite à partir de 100€ */
    livraison = 0
...
myForm.addEventListener('submit', function(e){ /* stocker le tableau product et tableau dans le localstorage */
  e.preventDefault()
  let products = []
...
if(produit){ /* suppression d'un produit */
  for (i=0 ; i<produit.length; i++){
...
let totalPanier = produit.length /* stocker le nombre de produit dans le panier dans le localstorage  */
localStorage.setItem("nombreDeProduit",totalPanier)
console.log(localStorage.nombreDeProduit)

if(!localStorage.nombreDeProduit){ /* inserer le nombre de produit dans le html  */
  document.getElementById("itemNumb").innerHTML = 0
} else {
